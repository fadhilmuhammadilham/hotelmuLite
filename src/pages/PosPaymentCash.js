  ...
  async action() {
    ...
    const sugestionPay = () => {
      if(this.basketService.items.length > 0){
        ...
        let totalPrices = this.transactionService.totalDiscount > 0 ? this.transactionService.totalAfterDiscount: this.transactionService.total_prices
        let total = Math.round(totalPrices / 10000.1) * 10000
        total = totalPrices > total ? total + 5000 : total
    ...
    if ($('#payment-suggestion').length > 0) sugestionPay()
		...
		$('a[data-action="other-nominal"]').on('click', () => {
